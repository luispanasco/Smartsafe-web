                ...
                <Container className="relative z-10">
                    <div className="grid lg:grid-cols-2 gap-6 md:gap-12 items-center">
                        <div className="order-2 lg:order-1">
                            ...
                            <h1 className="text-3xl md:text-6xl font-bold text-ui-text-primary mb-3 md:mb-6 font-[family-name:var(--font-space-grotesk)] leading-tight">
                                {product.title}
                            </h1>
                            <p className="text-base md:text-xl text-ui-text-secondary leading-relaxed mb-4 md:mb-8 max-w-xl line-clamp-4 md:line-clamp-none">
                        ...
                        <div className="order-1 lg:order-2 relative w-full">
                            <div className="absolute inset-0 bg-gradient-to-tr from-brand-primary/20 to-transparent rounded-full blur-3xl opacity-30 animate-pulse pointer-events-none" />
                        ...
                        <div className="space-y-8">
                                ...
                                <div className="space-y-6">
                                    {product.consultation.locations.map((loc) => (
                                        <div key={loc.slug} className="pt-4 border-t border-white/5 first:border-0 first:pt-0">
                                            <div className="flex items-center gap-2 mb-3 text-brand-accent font-semibold">
                                                <MapPin size={16} />
                                                <span>{loc.name}</span>
                                            </div>
                                            ...
                                            <div className="space-y-3">
                                                {loc.contacts.map((contact, idx) => (
                                                    <div key={idx} className="bg-ui-surface rounded-lg p-3 border border-white/5">
                                                        <p className="text-ui-text-primary text-sm font-medium mb-1">{contact.name}</p>

                                                        {/* Roles Chips */}
                                                        {Array.isArray(contact.role) ? (
                                                            <div className="flex flex-wrap gap-1.5 mb-2">
                                                                {contact.role.map((r, i) => (
                                                                    <span key={i} className="text-[10px] bg-ui-bg-secondary text-ui-text-muted px-1.5 py-0.5 rounded border border-white/5">
                                                                        {r}
                                                                    </span>
                                                                ))}
                                                            </div>
                                                        ) : (
                                                            <p className="text-ui-text-muted text-xs mb-2">{contact.role}</p>
                                                        )}
                            ...
                            {product.purchase && hasValidUrl(product.purchase.url) && (
                                <a
                                    ...
                                    className="flex items-center justify-center gap-2 w-full bg-action-primary-default text-white font-bold py-4 rounded-xl hover:bg-action-primary-hover transition-colors shadow-lg shadow-black/20"
                                >
                            ...
                            {/* Coverage Box */}
                            {product.coverage && (
                                <div className="bg-gradient-to-br from-brand-primary to-brand-secondary rounded-2xl p-6 text-white shadow-xl shadow-black/20">
                                    <h3 className="text-xl font-bold mb-2 font-[family-name:var(--font-space-grotesk)]">
                                        ¿Es compatible?
                                    </h3>
                                    <p className="text-ui-text-secondary text-sm mb-6">
                                    ...
                                    <a
                                        ...
                                        target="_blank"
                                        rel="noopener noreferrer"
                                        className="block w-full bg-white text-brand-primary text-center font-bold py-3 rounded-xl hover:bg-zinc-100 transition-colors"
                                    >
                                        Verificar Cobertura
                                    </a>
                                </div>
